...
Component({
  behaviors: [getBehavior(), yuvBehavior, cameraBehavior],
  data: {
    theme: 'light',
    cameraPosition: 1,
    buttonDisable: true,
  ...
  methods: {
    ...
    onCustomerUpdateAnchor(anchor) {
      this.processVKSessionFaceData(anchor)
    },
    onCustomerRemoveAnchor() {
      this.processVKSessionFaceLost() 
    },
    onRecordComplete(path) {
      console.log('record ok ' + path)
