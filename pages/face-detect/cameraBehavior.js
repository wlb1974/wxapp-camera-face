import { getAuthorize, setAuthorize, throttle, checkVersion } from './utils'

const tips = {
  ready: '请确保光线充足,正面镜头',
  recording: '人脸录制中..',
  complete: '已录制完成',
  error: '录制失败'
} 

const cameraBehavior = Behavior({
  // 提示信息

  properties: {
    // 人脸整体可信度 [0-1], 参考wx.faceDetect文档的res.confArray.global
    // 当超过这个可信度且正脸时开始录制人脸, 反之停止录制
    faceCredibility: {
      type: Number,
      value: 0.5
    },
    // 人脸偏移角度正脸数值参考wx.faceDetect文档的res.angleArray
    // 越接近0越正脸，包括p仰俯角(pitch点头）, y偏航角（yaw摇头), r翻滚角（roll左右倾）
    faceAngle: {
      type: Object,
      value: { p: 0.5, y: 0.5, r: 0.5 }
    },
    // 录制视频时长,不能超过30s
    duration: {
      type: Number,
      value: 10000
    },
    // 是否压缩视频
    compressed: {
      type: Boolean,
      value: false
    },
    // 前置或者后置 front,back
    devicePosition: {
      type: String,
      value: 'front'
    },
    // 指定期望的相机帧数据尺寸 small,medium,large
    frameSize: {
      type: String,
      value: 'medium'
    },
    // 分辨率 low,medium,high
    resolution: {
      type: String,
      value: 'medium'
    },
    // 闪光灯 auto,on,off,torch
    flash: {
      type: String,
      value: 'off'
    },
    // 检测视频帧的节流时间，默认500毫秒执行一次
    throttleFrequency: {
      type: Number,
      value: 500
    }
  },


  // 组件的初始数据
  data: {
    isReading: false, // 是否在准备中
    isRecoding: false, // 是否正在录制中
    isStopRecoding: false, // 是否正在停止录制中
    bottomTips: '', // 底部提示文字
    camera : null 
  },

  methods: {
    // 准备录制
    async readyRecord() {
      const result = await this.initAuthorize();
      if (!result) return false;
      this.ctx = wx.createCameraContext()
      if (this.data.isReading) return
      this.setData({ isReading: true , isRecoding:false , isCompleteRecoding: false, isStopRecoding: false})
      // wx.showLoading({ title: '加载中..', mask: true })

      console.log('准备录制')
      this.setData({ bottomTips: tips.ready })
    } ,

    // 处理人脸识别数据
    processVKSessionFaceData(anchor) {
      
      if (this.data.isRecoding || this.data.isCompleteRecoding) return
      if( !this.data.isReading) {
        return 
      }
      this.setData({ isRecoding: true });
      console.log('人脸可信,且是正脸 origin=' + JSON.stringify(anchor.origin) + "  size=" + JSON.stringify(anchor.size) + " angle=" + JSON.stringify(anchor.angle))  
      this.startRecord(); // 开始录制      
    },
    processVKSessionFaceLost() {
      if (!(this.data.isRecoding || this.data.isCompleteRecoding)) return
      console.log('人脸不可信,或者不是正脸');
      this.cancel() 
    }, 
    // 开始录制
    startRecord() {
      console.log('开始录制')
      this.setData({ isCompleteRecoding: false })
      try {
        this.ctx.startRecord({
          timeout: 200 ,    // 最长测试时间200S以内
          success: (res) => {
            this.setRecordingTips();
            this.timer = setTimeout(() => {
              this.completeRecord()
            }, this.properties.duration + 500) // 额外增加500ms
          },
          timeoutCallback: (res) => {
            // 超过30s或页面 onHide 时会结束录像
            this.completeRecord()
          },
          fail: () => this.stop() ,
          complete : () => {
            
          }
        })

      }
      catch(e) {
        console.error('startRecord error: ' , e)
        this.setData({isReading: false, isRecoding:false, isCompleteRecoding:false})
        setTimeout( () => {
          this.setData({ bottomTips: '' })
        } , 5000)
      }
    },
    // 设置录制中的提示文字和倒计时
    setRecordingTips() {
      let second = (this.properties.duration / 1000);
      if (this.interval) clearInterval(this.interval);
      this.interval = setInterval(() => {
        console.log('xxxxxx', second);
        this.setData({
          bottomTips: tips.recording + second-- + 's'
        })
        if (second <= 0) clearInterval(this.interval);
      }, 1000)
    },

    // 完成录制
    completeRecord() {
      console.log('完成录制');
      this.setData({ isCompleteRecoding: true })
      // wx.stopFaceDetect();
      if (this.interval) clearInterval(this.interval);

      console.log('calling stopRecord!')
      try {
        if(this.ctx) {
          console.log('call stopRecord!')
        }
        this.ctx.stopRecord({
          compressed: this.properties.compressed,
          success: (res) => {
            console.log('stopRecord success!')
            this.setData({ bottomTips: tips.complete })
            setTimeout( () => {
              this.setData({ bottomTips: '' })
            } , 5000)
          },
          fail: () => this.stop(),
          complete: (res) => {
            // this.listener.stop();
            console.log('complete stopRecord : ')
            if(this.interval)
              clearInterval(this.interval);
            // this.setData({ isCompleteRecoding: false })
            // 向外触发完成录制的事件
  
            console.log('sendEvent: complete')
            this.onRecordComplete(res.tempVideoPath)
            this.setData({isReading: false, isRecoding:false, isCompleteRecoding:false})
            // this.triggerEvent('complete', res.tempVideoPath)
          }
        })
      }
      catch(e) {
        console.log("stopRecord exception : " + e)
      }
     
    },
    // 人脸移出等取消录制
    cancel() {
      console.log('取消录制');
      // 如果不在录制中或者正在录制完成中就不能取消
      if (!this.data.isRecoding || this.data.isCompleteRecoding) return
      clearTimeout(this.timer);
      if(this.interval)
        clearInterval(this.interval);
      this.ctx.stopRecord({
        complete: () => {
          console.log('取消录制成功');
          this.setData({ bottomTips: tips.ready, isRecoding: false });
        }
      });
    },
    // 用户切入后台等停止使用摄像头
    stop() {
      console.log('停止录制');
      clearTimeout(this.timer);
      if(this.interval)
        clearInterval(this.interval);
      // if(this.listener) this.listener.stop();
      if (this.ctx && !this.data.isCompleteRecoding) this.ctx.stopRecord()
      // wx.stopFaceDetect();
      setTimeout(() => {
        this.setData({ bottomTips: '', isRecoding: false })
      }, 500)
    },
    // 用户不允许使用摄像头
    error(e) {
      console.log('录制异常：' + JSON.stringify(e));
      // const cameraName = 'scope.camera';
      // this.triggerEvent('noAuth', cameraName)
    },

    // 初始相机和录音权限
    async initAuthorize() {
      const cameraName = 'scope.camera';
      const recordName = 'scope.record';
      const scopeCamera = await getAuthorize(cameraName);
      // 未授权相机
      if (!scopeCamera) {
        // 用户拒绝授权相机
        if (!(await setAuthorize(cameraName))) this.openSetting();
        return false;
      }
      const scopeRecord = await getAuthorize(recordName);
      if (!scopeRecord) {
        // 用户拒绝授权录音
        if (!(await setAuthorize(recordName))) {
          this.openSetting();
          return false;
        }
      }
      return true;
    },

    // 打开设置授权
    openSetting() {
      wx.showModal({
        title: '开启摄像头和录音权限',
        showCancel: true,
        content: '是否打开？',
        success: (res) => {
          this.triggerEvent('noAuth', '打开设置授权')
          if (res.confirm) {
            wx.openSetting();
          }
        }
      });
    }
  }
}) 

export default cameraBehavior